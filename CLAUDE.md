...
**Kafka 브로커/토픽과 MinIO·Neo4j 는 K8s 에 없다** — 브로커는 ADR 0 에 따라
Ansible(`kafka_ansible.yml`)이 노드 로컬(systemd)에 설치하고, MinIO·Neo4j 도 같은 방식의
로컬 설치다. **PostgreSQL 은 CNPG 로 K8s 에 복귀했다**(103-cnpg 오퍼레이터 + 303-postgres
Cluster — 구 Ansible postgres 롤은 퇴역, 중지 보존). 파이프라인은 공용 ConfigMap 의 접속값
(`KAFKA_BOOTSTRAP`·`MINIO_S3_ENDPOINT`·`COLLECTOR_DB_HOST`·`PLATFORM_NEO4J_URI`)으로 붙고,
이 중 `COLLECTOR_DB_HOST` 만 노드 주소가 아니라 CNPG rw Service FQDN 이다.
...
- `300-data-layer-base` — **네임스페이스 `data-layer` + 공용 ConfigMap `data-layer-env` + Secret `data-layer-secrets` + 권한 바인딩의 소유자.** 워크로드가 없고 이미지도 쓰지 않는다. Kafka 브로커와 MinIO·Neo4j 는 ADR 0 에 따라 노드 로컬 설치 — 접속값은 values.yaml(`global.kafkaBootstrap`·`minioEndpoint`·`neo4jBoltUri`)로 조립하며 Ansible `host.yml`·실제 설치 위치와 일치해야 한다. **`global.postgresHost` 만 예외로 노드 주소가 아니다** — 303-postgres 의 CNPG rw Service FQDN 이고, 304 의 `postgres_host`·303 의 `clusterName` 과 같은 커밋 규칙이다.
...
- **리소스가 없는 프로바이더는 선언하지 않는다.** helm 프로바이더는 서드파티 차트를 쓰는 스택(100/101/102/200)에만 둔다. 102 는 차트(helm) + MetalLB CR(kubernetes)이라 둘, 200 은 차트(helm) + Harbor API(harbor) 둘이다.
...
- **NodePort 는 더 이상 없다.** 비-HTTP 노출은 인그레스 대상이 아니고, 남은 사례는 셋이다:
